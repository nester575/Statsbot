...
def aggregate_reports(rows):
    ...
    for r in rows:
        sp = r["specialist"]
        bucket = result.setdefault(sp, {"metrics": {}, "comments": [], "_days": set()})
...
async function loadAggregate(period){
  ...
  for(const n of SP){
    ...
    if(metricsArr.length){
      h+='<table>';
      metricsArr.forEach(([m,v])=>{h+=`<tr><td style="color:#5a6070">${esc(m)}</td><td><span class="num">${fmtNum(v)}</span></td></tr>`});
      h+='</table>';
    }else{
